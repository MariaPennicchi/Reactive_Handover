...
class FrankaMoveitControl:
    def __init__(self):
        ...
        self.event_reader = rospy.Subscriber("/event", String, self.moving_away, queue_size=10)
        self.event_publisher = rospy.Publisher("/event", String, queue_size=10)

        self.motion_in_progress = False
        self.target_pose = None
        self.last_target_pose = None
        self.reactive_thr = 0.1
        self.last_msg = None
        self.planning_times = []

        self.state = ""
        self.state_reader = rospy.Subscriber('/state', String, self.process_state, queue_size=1)
        self.pose_sub_target = rospy.Subscriber('/pose_target_typed', PoseStampedTyped, self.compute_target)
        ...
        self.ts = message_filters.TimeSynchronizer([self.bounding_box_reader], 1)
    ...
    def transform_joint(self, msg):
        ...
        for link in self.link_names:
            try:
                ...
                rospy.logwarn(f"Transform di {link} fallita: {e}")
                continue
            except Exception as e:
                rospy.logwarn(f"Errore nel calcolo della posa di {link}: {e}")
                continue
    ...
    def compute_target(self, msg):
        if self.state == "TRACKING" or self.state == "MOVING" or self.state == "REACHING":
    ...
    def handle_pose_target(self, msg):
        if self.state == "TRACKING" and self.motion_in_progress:
            ...
            self.last_target_pose = self.target_pose
            self.handle_pose_target(self.last_msg)
        elif self.state == "MOVING" and self.last_target_pose is not None and self.target_pose is not None and np.linalg.norm([self.target_pose.position.x - self.last_target_pose.position.x, self.target_pose.position.y - self.last_target_pose.position.y, self.target_pose.position.z - self.last_target_pose.position.z]) > self.reactive_thr:
            self.event_publisher.publish("hand_moved")
            rospy.loginfo("hand moved due to target pose------------------")
            ...
            rospy.loginfo("Target Pose to reach: (%.3f, %.3f, %.3f)",self.target_pose.position.x,
                          self.target_pose.position.y, self.target_pose.position.z)
            '''

    def check_target_reached(self, msg):
        if (self.state == "MOVING" or self.state == "REACHING") and self.target_pose is not None:
            if self.poses_are_close(self.move_group.get_current_pose().pose, self.target_pose):
                # rospy.loginfo("Current Pose reached: (%.3f, %.3f, %.3f)", self.move_group.get_current_pose().pose.position.x,
                #               self.move_group.get_current_pose().pose.position.y,
                #               self.move_group.get_current_pose().pose.position.z)
                # rospy.loginfo("Target Pose reached: (%.3f, %.3f, %.3f)", self.target_pose.position.x,
                #               self.target_pose.position.y, self.target_pose.position.z)
                self.move_group.stop()
                self.move_group.clear_pose_targets()
                ...
                self.event_publisher.publish("end") #comment this to detach the MOVING DOWN module
                self.motion_in_progress = False     #comment this to detach the MOVING DOWN module
    ...
    def moving_away(self, msg):
        ...
        if event == "moving_away_to_home":
            rospy.loginfo('Moving away to home.')
            self.move_group.stop()
            self.move_group.clear_pose_targets()
            self.motion_in_progress = True
            initial_pose = [0.0, -0.785, 0.0, -2.356, 0.0, 1.571, 0.785]  # Default home pose
